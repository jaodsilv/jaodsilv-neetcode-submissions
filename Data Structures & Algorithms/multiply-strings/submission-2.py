class Solution:
    def multiply(self, num1: str, num2: str) -> str:
        ...
        print(res)
        for i in range(1, len(n2)): # O(10*len(n1)) + O(len(n2)) + O(len(n2)² + len(n1)*len(n2)) = O(len(n2)² + len(n1)*len(n2)) = O(min(len(num1), len(num2))² + len(num1)*len(num2)) = O(len(num1)*len(num2))
